...
class hkxNode : public hkxAttributeHolder
{
	...
	public:
		...
		hkStringPtr m_name;
			...
			/// Will assume a reference fo the child (will remove ref on dtor)
		hkRefVariant m_object;
		
			/// Raw keyframe data
		hkArray< hkMatrix4 > m_keyFrames;
		
		/// The children of this node. This link forms the scene graph. Will assume a reference fo the child (will remove ref on dtor)
