...
int hkSerializeMultiMap<KEY,VALUE>::getFreeIndex()
{
	int i;
...
template <typename KEY, typename VALUE>
int hkSerializeMultiMap<KEY,VALUE>::getFirstIndex( KEY k ) const
{
	return m_indexMap.getWithDefault(k, -1);
}

template <typename KEY, typename VALUE>
hkBool32 hkSerializeMultiMap<KEY,VALUE>::insert( KEY k, const VALUE& value )
{
...
template <typename KEY, typename VALUE>
int hkSerializeMultiMap<KEY,VALUE>::addPendingValue( const VALUE& value, int nextIndex )
{
	int thisIndex = getFreeIndex();
...
template <typename KEY, typename VALUE>
void hkSerializeMultiMap<KEY,VALUE>::realizePendingKey( KEY k, int index )
{
...
template <typename KEY, typename VALUE>
int hkSerializeMultiMap<KEY,VALUE>::getNumKeys() const
{
	return m_indexMap.getSize();
}

template <typename KEY, typename VALUE>
void hkSerializeMultiMap<KEY,VALUE>::clear()
{
	m_valueChain.clear();
	m_indexMap.clear();
	m_freeChainStart = -1;
...
template <typename KEY, typename VALUE>
int hkSerializeMultiMap<KEY,VALUE>::removeByIndex( KEY key, int removeIndex )
{
#ifdef HK_DEBUG
	{
...
template <typename KEY, typename VALUE>
void hkSerializeMultiMap<KEY,VALUE>::removeByValue( KEY key, const VALUE& v )
{
...
template <typename KEY, typename VALUE>
void hkSerializeMultiMap<KEY,VALUE>::removeKey( KEY key )
{
	hkPointerMap<void*, int>::Iterator it = m_indexMap.findKey(key);
...
template <typename KEY, typename VALUE>
void hkSerializeMultiMap<KEY,VALUE>::changeKey( KEY oldKey, KEY newKey )
{
