...
class hkQemSimplifier: public hkReferencedObject
{
	public:
		...
		struct Group
		{
			...
			int m_attributeSize;
			int m_attributeVecSize;								///

			int m_quadricSize;									///
			int m_quadricVecSize;								///

			hkReal m_positionToAttribScale;						///

			hkArray<int> m_availableAttributeIndices;			///
			hkArray<hkVector4*> m_attributes;					///
			hkFreeList m_attributeFreeList;						///
		};
	...
	protected:
		...
		hkBool _areGroupIndicesValid(int groupIndex) const;
			...
			/// Trys to calculate best contraction position - if fails falls back on just choosing the start or end
		...
		hkFreeList m_contractionFreeList;										///
		hkMinHeap<EdgeContraction*, EdgeContraction> m_contractions;			///
		hkPointerMap<hkUint64, EdgeContraction*> m_edgeContractionMap;			///

		hkArray<Group> m_groups;												///

		hkgpVertexTriangleTopology<Triangle> m_topology;						///

		hkAabb m_aabb;															///
		hkVector4 m_aabbScale;														///
