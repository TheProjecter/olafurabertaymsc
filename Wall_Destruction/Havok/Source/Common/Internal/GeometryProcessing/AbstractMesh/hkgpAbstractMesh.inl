...
//
template <typename EDGE,typename VERTEX,typename TRIANGLE,typename ALLOCATOR>
inline void					hkgpAbstractMesh<EDGE,VERTEX,TRIANGLE,ALLOCATOR>::EdgeIterator::next()
{
	for(;m_edge.isValid();)
	{
		do
...
//
template <typename EDGE,typename VERTEX,typename TRIANGLE,typename ALLOCATOR>
inline						hkgpAbstractMesh<EDGE,VERTEX,TRIANGLE,ALLOCATOR>::~hkgpAbstractMesh()
{
#if defined(HK_DEBUG)
	reset();
#endif
...
//
template <typename EDGE,typename VERTEX,typename TRIANGLE,typename ALLOCATOR>
inline	void				hkgpAbstractMesh<EDGE,VERTEX,TRIANGLE,ALLOCATOR>::reset()
{
#if defined(HK_DEBUG)
...
//
template <typename EDGE,typename VERTEX,typename TRIANGLE,typename ALLOCATOR>
inline bool					hkgpAbstractMesh<EDGE,VERTEX,TRIANGLE,ALLOCATOR>::isValid() const
{
...
//
template <typename EDGE,typename VERTEX,typename TRIANGLE,typename ALLOCATOR>
inline typename hkgpAbstractMeshDefinitions::Error	hkgpAbstractMesh<EDGE,VERTEX,TRIANGLE,ALLOCATOR>::checkTopology(bool checkForOrphanVertices) const
{
...
//
template <typename EDGE,typename VERTEX,typename TRIANGLE,typename ALLOCATOR>
inline bool					hkgpAbstractMesh<EDGE,VERTEX,TRIANGLE,ALLOCATOR>::hasReferenceTo(const VERTEX* vertex) const
{
	for(TRIANGLE* t=m_triangles.getFirst();t;t=t->next())
	{
...
//
template <typename EDGE,typename VERTEX,typename TRIANGLE,typename ALLOCATOR>
inline bool					hkgpAbstractMesh<EDGE,VERTEX,TRIANGLE,ALLOCATOR>::isClosedManifold() const
{
	for(TRIANGLE* t=m_triangles.getFirst();t;t=t->next())
	{
...
//
template <typename EDGE,typename VERTEX,typename TRIANGLE,typename ALLOCATOR>
inline bool					hkgpAbstractMesh<EDGE,VERTEX,TRIANGLE,ALLOCATOR>::bindEdge(Edge edge) const
{
	for(TRIANGLE* t=m_triangles.getFirst();t;t=t->next())
	{
		for(int i=0;i<3;++i)
...
//
template <typename EDGE,typename VERTEX,typename TRIANGLE,typename ALLOCATOR>
inline EDGE					hkgpAbstractMesh<EDGE,VERTEX,TRIANGLE,ALLOCATOR>::findEdgeOrdered(const VERTEX* a,const VERTEX* b) const
{
	for(TRIANGLE* t=m_triangles.getFirst();t;t=t->next())
	{
...
//
template <typename EDGE,typename VERTEX,typename TRIANGLE,typename ALLOCATOR>
inline EDGE					hkgpAbstractMesh<EDGE,VERTEX,TRIANGLE,ALLOCATOR>::findEdgeUnordered(const VERTEX* a,const VERTEX* b) const
{
	for(TRIANGLE* t=m_triangles.getFirst();t;t=t->next())
	{
		for(int i=0;i<3;++i)
...
//
template <typename EDGE,typename VERTEX,typename TRIANGLE,typename ALLOCATOR>
inline TRIANGLE*			hkgpAbstractMesh<EDGE,VERTEX,TRIANGLE,ALLOCATOR>::findTriangleUnordered(const VERTEX* a,const VERTEX* b,const VERTEX* c) const
{
	for(TRIANGLE* t=m_triangles.getFirst();t;t=t->next())
	{
		int	mask=0;
