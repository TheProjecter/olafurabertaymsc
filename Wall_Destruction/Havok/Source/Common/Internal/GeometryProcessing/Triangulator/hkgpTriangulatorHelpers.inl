...
inline bool						TRIANGULATOR_TYP::read(const char* filename)
{
	...
	if(file.isOk())
	{
		...
		if(file.read32()!=cfgOrientation) HK_ERROR(0x980652E6,"Incompatible orientation");
...
TRIANGULATOR_TYP*				TRIANGULATOR_TYP::benchmark()
{
	struct Utils
	{
		HK_FORCE_INLINE static Insertion	insertVertex(hkTriangulator* i,int x,int y,bool cf)
		{
		...
		HK_FORCE_INLINE static Insertion	insertEdge(hkTriangulator* i,const Vertex* v0,const Vertex* v1,bool cf)
		{
		...
		HK_FORCE_INLINE static void			insertRandom(hkTriangulator* instance,int count,int margin,bool conform,hkPseudoRandomGenerator& rndgen)
		{
	...
	hkGeomTriangulator*			instance=new hkTriangulator();
...
#endif
	/* Infos											*/ 
	printf("Size of triangulator: %u bytes\r\n",sizeof(hkTriangulator));
	printf("Size of vertex:       %u bytes\r\n",sizeof(hkTriangulator::Vertex));
	printf("Size of triangle:     %u bytes\r\n",sizeof(hkTriangulator::Triangle));
	printf("Roots:                %u^2\r\n",hkTriangulator::cfgNumRoots);
	printf("Cycles:               %u\r\n",nVCycles);
	...
	{		
		...
		for(int pass=0;pass<nVCycles;++pass)
		{
			...
			for(int i=0,ni=vertices.getSize();i<ni;++i)
			{
				instance->removeVertex(vertices[i]->m_x,vertices[i]->m_y,true);
			}
		}
		clock.print(nVCycles*nVInsertions);
	}
