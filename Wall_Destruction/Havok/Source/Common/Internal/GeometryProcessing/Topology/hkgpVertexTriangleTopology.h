...
class hkgpVertexTriangleTopologyBase
{
    public:
		...
		hkBool32 isOk() const;
			...
			/// For each vertex specifies the new vertex index (ie size must be as big as the amount of vertices)
    ...
    protected:
        ...
        hkFreeList m_triangleFreeList;							///
        hkArray<Triangle*> m_triangles;							///
		VertexEdgeMapType m_vertexEdgeMap;			///
};
