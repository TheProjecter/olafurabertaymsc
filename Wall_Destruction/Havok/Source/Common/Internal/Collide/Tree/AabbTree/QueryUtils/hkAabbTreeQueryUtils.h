...
#define TreeNode typename hkAabbTree<BOUNDING_VOLUME, hkIdxType>::Node
#define hkDynamicAabbTree hkAabbTree<BOUNDING_VOLUME, hkIdxType>
...
class hkAabbTreeQueryUtils
{
	...
	private:
		static int HK_CALL HK_AABB_TREE_FUNCTION( queryAabbInternal )( const hkDynamicAabbTree* tree, const hkAabb& aabb, 
			Collector* collector, hkArray<void*>* ptrArray, voidWriteIterator* hitArray, int hitCapacity );

};

#undef TreeNode
#undef hkDynamicAabbTree
