...
#if defined(HK_PLATFORM_SPU) || defined(HK_PLATFORM_RVL) || defined(HK_PLATFORM_PSP) || (defined(HK_PLATFORM_WIN32) && (HK_CONFIG_SIMD == HK_CONFIG_SIMD_DISABLED))
...
HK_FORCE_INLINE hkAabbTreeData::RayBundle::RayBundle( const Bundle& from, const Bundle& to, hkVector4Comparison::Mask mask )
{
	...
	for(int i=0; i<3; i++)
	{
		...
		hkVector4 maskedDelta; maskedDelta.select32( delta.m_data[i], (hkVector4&)hkQuadReal1111, isZero );
		hkVector4 invDelta;
		invDelta.setReciprocal4( maskedDelta );
		m_invDir.m_data[i].select32( invDelta, (const hkVector4&)QuadRealMax_Const, isZero );
	}
}
