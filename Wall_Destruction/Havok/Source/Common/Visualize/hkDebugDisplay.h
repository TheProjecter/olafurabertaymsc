...
class hkDebugDisplay : public hkReferencedObject, public hkSingleton<hkDebugDisplay>
{
	...
	public:
		...
		void displayModelSpacePose(int numTransforms, const hkInt16* parentIndices, const hkQsTransform* modelSpacePose, const hkQsTransform& worldFromModel, int id, int tag, int color = 0x7fffffff, hkReal boneAxisSize = 0.25f, hkBool showRootTranslation = false);

			/// Display a local space animation pose.  Please use the HK_DISPLAY_LOCAL_SPACE_POSE macro instead of
			/// calling this function directly.		
		void displayLocalSpacePose(int numTransforms, const hkInt16* parentIndices, const hkQsTransform* localSpacePose, const hkQsTransform& worldFromModel, int id, int tag, int color = 0x7fffffff, hkReal boneAxisSize = 0.25f, hkBool showRootTranslation = false);
