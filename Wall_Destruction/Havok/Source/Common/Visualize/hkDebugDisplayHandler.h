...
class hkDebugDisplayHandler
{
	public:
		...
		//
		virtual hkResult updateBehavior( const hkArrayBase<int>& wordVarIdx, const hkArrayBase<int>& wordStack, const hkArrayBase<int>& quadVarIdx, const hkArrayBase<hkVector4>& quadStack,
			const hkArrayBase<char*>& activeNodes, const hkArrayBase<int>& activeStateIds, const hkArrayBase<int>& activeTransitions, const hkArrayBase<hkQsTransform>& transforms) = 0;
