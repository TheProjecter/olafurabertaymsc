...
class hkSpuUtil : public hkReferencedObject
{
	public:
		...
		/// Joins all the SPURS tasks in the Havok taskset
		void joinSpursTasks();
...
#  define HK_DISABLE_SPU_INTERRUPTS  unsigned int interruptsEnabled = spu_readch(SPU_RdMachStat) & 1; spu_idisable();
#  define HK_ENABLE_SPU_INTERRUPTS   if ( interruptsEnabled ) spu_ienable();
#else
