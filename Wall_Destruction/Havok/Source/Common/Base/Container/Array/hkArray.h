...
#include <Common/Base/Container/hkContainerAllocators.h>
	...
	///	<br>
	/// The hkArray uses Havok type traits for optimizated handling the POD (plain old data)
	/// and non-POD items.<p>
	///
	/// Object lifetimes: Memory at elements [0, getSize()) are live objects.
	/// Memory at elements [getSize(),getCapacity()) is uninitialized.<p>
	///
	/// Note: If the type of the array item is non-pod then the following must be
	/// publicly accessible:
	/// <ul><li>user-defined default constructor</li>
	/// <li>user-defined copy constructor</li>
	/// <li>user-defined destructor</li>
	/// <li>user-defined assignment</li></ul>
	/// If the class contains member of hkArray type you must implement copy constructor,
	/// compiler should generate an error if the copy constructor is trivial.<p>
	///
	...
	/// <ul><li>HK_DECLARE_POD_TYPE();<br>
	/// It must be specified in public area of the class body.</li>
	/// <li>HK_DECLARE_POD_TYPE_IF_POD( T );<br>
	/// It must be specified in public area of the class template body, where the T
	/// is the template argument.</li>
	/// <li>HK_DEFINE_AS_POD_TYPE( typename );<br>
	/// It must be specified outside the class body. This macro can be used in
	/// the cpp file to define the class as a pod type locally.</li></ul>
...
class hkArray : public hkArrayBase<T>
{
	...
	public:
		...
		HK_FORCE_INLINE void useExternalArray(T* t, int numElements);
		HK_FORCE_INLINE void useExternalBuffer(void* buf, int numBytes);
