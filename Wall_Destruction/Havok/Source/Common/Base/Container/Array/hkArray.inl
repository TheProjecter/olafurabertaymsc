...
template <typename T>
void hkArrayBase<T>::_useExternalArray(T* buf, int capacity)
{
...
template <typename T>
void hkArrayBase<T>::_useExternalBuffer(void* buf, int numBytes)
{
	_useExternalArray( static_cast<T*>(buf), numBytes/sizeof(T) );
}

template <typename T, typename Allocator>
void hkArray<T,Allocator>::useExternalArray(T* t, int numElements)
{
	_clearAndDeallocate(AllocatorType().get(this));
	hkArrayBase<T>::_useExternalArray(t, numElements);
}

template <typename T, typename Allocator>
void hkArray<T,Allocator>::useExternalBuffer(void* buf, int numBytes) 
{
	_clearAndDeallocate(AllocatorType().get(this));
	hkArrayBase<T>::_useExternalBuffer(buf, numBytes);
}
...
HK_FORCE_INLINE hkArrayBase<T>::~hkArrayBase()
{
	HK_ASSERT2(0x1129f768, m_size==0, "Array elements not destructed");
...
template <typename T>
void hkArrayBase<T>::setOwnedData(T* ptr, int size, int capacity)
{
	HK_ASSERT(0x23483be5, size >= 0 );
	HK_ASSERT(0x23483be5, size <= (capacity&~DONT_DEALLOCATE_FLAG) );
	m_data = ptr;
	m_size = size;
...
: hkArrayBase<T>()
{
	...
	hkArrayBase<T>::setOwnedData( p, n, cap);
	hkArrayUtil::construct( hkArrayBase<T>::m_data, n, typename hkIsPodType<T>::type());
}
...
: hkArrayBase<T>()
{
	...
	int cap = n ? n : hkArrayBase<T>::DONT_DEALLOCATE_FLAG;
	hkArrayBase<T>::setOwnedData( p, n, cap);
	hkArrayUtil::constructWithCopy( hkArrayBase<T>::m_data, hkArrayBase<T>::m_size, t, typename hkIsPodType<T>::type());
}
...
: hkArrayBase<T>()
{
	...
	int cap = n ? n : hkArrayBase<T>::DONT_DEALLOCATE_FLAG;
	hkArrayBase<T>::setOwnedData( p, n, cap);
	hkArrayUtil::constructWithArray( hkArrayBase<T>::m_data, hkArrayBase<T>::m_size, arr.m_data, typename hkIsPodType<T>::type());
}

template <typename T, typename Allocator>
HK_FORCE_INLINE hkArray<T, Allocator>& hkArray<T, Allocator>::operator= (const hkArrayBase<T>& a)
{
	copyFromArray(Allocator().get(this), a, typename hkIsPodType<T>::type());
	return *this;
}
