...
:	m_ops(ops)
{
	...
	m_elem = mem._blockAlloc<hkUlong>( initialCapacity*3 );
...
{
	hkMemoryAllocator& mem = Allocator().get(this);
	mem._blockFree<hkUlong>( m_elem, (m_hashMod+1)*3 );
}
...
hkCachedHashMap<Operations, Allocator>& hkCachedHashMap<Operations, Allocator>::operator=(const hkCachedHashMap* other)
{
	...
	m_elem = mem._blockAlloc<hkUlong>( (m_hashMod+1)*3 );
...
void hkCachedHashMap<Operations, Allocator>::resizeTable(int newcap)
{
	...
	hkMemoryAllocator& mem = Allocator().get(this);
	m_elem = mem._blockAlloc<hkUlong>(newcap*3); // space for values too
	...
	mem._blockFree<hkUlong>( oldelem, oldcap*3 );
}
