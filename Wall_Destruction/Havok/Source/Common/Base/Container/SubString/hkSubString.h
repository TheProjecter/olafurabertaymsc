...
/// the end pointer points to a character one past the end of the string. Ie m_end - m_start = num chars.
...
struct hkSubString
{
	public:
		...
		hkBool operator==(const hkSubString& rhs) const;
			/// != 
		HK_FORCE_INLINE bool operator!=(const hkSubString& rhs) const { return !(*this == rhs); }
