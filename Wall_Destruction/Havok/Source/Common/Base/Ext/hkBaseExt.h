...
#define EXT_DECLARE_REFOBJECT_ALLOCATOR() \
	HK_FORCE_INLINE void* HK_CALL operator new(hk_size_t nbytes)	{ HK_ASSERT_OBJECT_SIZE_OK(nbytes); hkReferencedObject* b = static_cast<hkReferencedObject*>(extAllocator::getInstance().blockAlloc(static_cast<int>(nbytes))); b->m_memSizeAndFlags = static_cast<hkUint16>(nbytes); HK_MEMORY_TRACKER_ON_NEW_REFOBJECT(nbytes, b); return b; } \
	HK_FORCE_INLINE void  HK_CALL operator delete(void* p)			{ hkReferencedObject* b = static_cast<hkReferencedObject*>(p); HK_MEMORY_TRACKER_ON_DELETE_REFOBJECT(b); extAllocator::getInstance().blockFree(p, b->m_memSizeAndFlags); } \
...
#define EXT_DECLARE_NONVIRTUAL_ALLOCATOR_BY_SIZE(CLASS_SIZE) \
	HK_FORCE_INLINE void* HK_CALL operator new(hk_size_t nbytes)	{ HK_ASSERT(0x6c787b7f, nbytes == CLASS_SIZE ); void* obj = extAllocator::getInstance().blockAlloc(static_cast<int>(nbytes)); HK_MEMORY_TRACKER_ON_NEW_OBJECT(CLASS_TYPE, nbytes, obj); return obj; } \
...
#define EXT_DECLARE_NONVIRTUAL_ALLOCATOR_BY_TYPE(CLASS_TYPE) \
	HK_FORCE_INLINE void* HK_CALL operator new(hk_size_t nbytes)	{ HK_ASSERT(0x6c787b7f, nbytes == sizeof(CLASS_TYPE) ); void* obj = extAllocator::getInstance().blockAlloc(int(sizeof(CLASS_TYPE))); HK_MEMORY_TRACKER_ON_NEW_OBJECT(CLASS_TYPE, nbytes, obj); return obj; } \
	HK_FORCE_INLINE void  HK_CALL operator delete(void* p)			{ if (p) { HK_MEMORY_TRACKER_ON_DELETE_OBJECT(p); extAllocator::getInstance().blockFree(p, sizeof(CLASS_TYPE)); }  } \
