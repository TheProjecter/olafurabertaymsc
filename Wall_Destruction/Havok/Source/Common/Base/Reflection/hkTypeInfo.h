...
class hkTypeInfo
{
	public:
		...
		CleanupLoadedObjectFunction getCleanupFunction() const
		{
			...
			/// Get the size of an instanciation in bytes
		hk_size_t getSize() const { return m_size; }

			/// Get the cpp type (or HK_NULL if not available)
		const hkCppTypeInfo* getCppType() const { return m_cppType; }
...
#define HK_REFLECTION_DEFINE_COMMON( TEXT_CLASS, SCOPED_CPP_CLASS, FINISH, CLEANUP, VTABLE ) \
	...
	const hkTypeInfo TEXT_CLASS##TypeInfo( #TEXT_CLASS, FINISH, CLEANUP, VTABLE, sizeof(SCOPED_CPP_CLASS), HK_GET_TYPE_ID(SCOPED_CPP_CLASS) )
