...
// The hkLargeBlockAllocator implements the hkFreeListAllocatorServer interface

#include <Common/Base/Memory/Allocator/FreeList/hkFreeListAllocatorServer.h>
...
class hkMemoryBlockServer
{
	public:
		...
		hk_size_t getMinAllocSize() const { return m_minAllocSize; }
			...
			/// with virtual memory its effectively infinite) the method will return hkMemoryStatistics::INFINTITE_SIZE
...
class hkLargeBlockAllocator:public hkFreeListAllocatorServer
{
    public:
		...
		void init(hkMemoryBlockServer* server) { m_server = server; }
        ...
        virtual hk_size_t getAllocatedSize(const void* p, hk_size_t size);	///
        virtual hk_size_t getAllocatedSizeWithOverhead(void* p, hk_size_t size);
        ...
        hkBool checkAllocations(void** allocs,int size);
            ...
            /// Get the statistics
        void calculateStatistics(hkMemoryAllocator::MemoryStatistics& stats);
