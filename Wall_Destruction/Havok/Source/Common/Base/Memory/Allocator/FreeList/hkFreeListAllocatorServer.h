...
class hkFreeListAllocatorServer
{
    public:
        ...
        virtual hkBool isValidAlloc(void* data) = 0;
            ...
            /// size passed in.
        virtual hk_size_t getAllocatedSize(const void* p, hk_size_t size) = 0;
            ...
            /// allocation or the size returned from getAllocatedSize.
        virtual hk_size_t getAllocatedSizeWithOverhead(void* p, hk_size_t size) = 0;
...
/// the hkFreeListAllocator system.
///
...
/// as is the case inside of the hkFreeListAllocator implementation

class hkDefaultFreeListMemoryServer: public hkReferencedObject, public hkSingleton<hkDefaultFreeListMemoryServer>,public hkFreeListAllocatorServer
{
    public:
        ...
        /// implementing hkFreeListAllocatorServer
        virtual void* alloc(hk_size_t size);
        virtual void free(void* data);
        virtual hkBool isValidAlloc(void* data) { return true; }
        virtual hk_size_t getAllocatedSize(const void* p, hk_size_t size) { return size; }
        virtual hk_size_t getAllocatedSizeWithOverhead(void* p, hk_size_t size) { return size; }
...
class hkAllocatorFreeListMemoryServer: public hkFreeListAllocatorServer
{
public:
	HK_DECLARE_PLACEMENT_ALLOCATOR();
	hkAllocatorFreeListMemoryServer(hkMemoryAllocator* allocator)
		: m_allocator(allocator)
	{
	}

	/// implementing hkFreeListAllocatorServer
	virtual void* alloc(hk_size_t size) { return hkMemoryRouter::easyAlloc(*m_allocator, int(size) ); }
	virtual void free(void* data) { hkMemoryRouter::easyFree(*m_allocator, data); }
	virtual hkBool isValidAlloc(void* data) { return true; }
	virtual hk_size_t getAllocatedSize(const void* p, hk_size_t size) { return size; }
	virtual hk_size_t getAllocatedSizeWithOverhead(void* p, hk_size_t size) { return size; }
	virtual void* criticalAlloc(hk_size_t size) { return alloc(size); }
