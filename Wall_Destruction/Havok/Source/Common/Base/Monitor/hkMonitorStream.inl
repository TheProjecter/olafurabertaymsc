...
#	define HK_MONITOR_MEMORY_INTERNAL(TAG, PTR, NBYTES) { \
	hkMonitorStream& mStream = hkMonitorStream::getInstance(); \
	if ( mStream.memoryAvailable() ) \
		{ \
		hkMonitorStream::MemoryCommand* h = reinterpret_cast<hkMonitorStream::MemoryCommand*>(mStream.getEnd()); \
		h->m_commandAndMonitor = TAG; \
		h->m_ptr = PTR; \
		h->m_sizeAndFlags = NBYTES; \
		mStream.setEnd( (char*)(h+1) );	\
} }
#	define HK_MONITOR_MEMORY_ALLOC(TAG, PTR, NBYTES) HK_MONITOR_MEMORY_INTERNAL(TAG,PTR,NBYTES)
#	define HK_MONITOR_MEMORY_FREE(TAG, PTR, NBYTES) HK_MONITOR_MEMORY_INTERNAL(TAG,PTR,(NBYTES)|(1<<31))
