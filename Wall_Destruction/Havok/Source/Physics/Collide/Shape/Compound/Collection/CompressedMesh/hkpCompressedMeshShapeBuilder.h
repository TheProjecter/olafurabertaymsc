...
class hkpCompressedMeshShapeBuilder
{
	public:
		...
		static hkpCompressedMeshShape* HK_CALL createMeshFromGeometry( const hkGeometry& geometry, hkReal quantizationError, 
			hkpCompressedMeshShape::MaterialType materialType, hkArray<hkpShapeKey>* shapeKeys = HK_NULL );
			...
			m_weldTolerance( 1e-3f )
		{
			...
			/// You need to specify a quantization  error and a material type (none, 1, 2 or 4 bytes per triangle).
		hkpCompressedMeshShape* createMeshShape( hkReal quantizationError, hkpCompressedMeshShape::MaterialType materialType );
		...
		void addGeometry( const hkGeometry& geometry, const hkMatrix4& transform, hkpCompressedMeshShape* compressedMesh,  
			hkArray<hkpShapeKey>* shapeKeys = HK_NULL );
		...
		void addInstance( int subpartID, const hkMatrix4& transform, hkpCompressedMeshShape* compressedMesh );
		...
		class MappingTree;
			...
			/// \mapping optional parameter for build the mapping tree to the original triangles
		...
		//
			...
			/// Converts an hkpCompressedMeshShape to an hkGeometry (not up to date)
		...
		static void HK_CALL convexPieceToGeometry( hkpCompressedMeshShape* compressedMesh, int index, hkGeometry& geometry );
		...
		static void HK_CALL appendGeometry( hkGeometry& dest, const hkGeometry& geom );

		
		// Inspects a geometry and outputs a material table and a recommended material type for building the compressed mesh
		//static void HK_CALL convertMaterials( hkArray<hkGeometry>& geometriesInOut, hkArray<hkUint32>& materialTableOut, 
		//	hkpCompressedMeshShape::MaterialType& materialTypeOut );
	...
	public:
		...
		struct TriangleMapping
		{
			...
			int m_originalIndex;
				...
				/// The position of the first index of the triangle
			int m_triangleIndex;
		...
		{
			// chunk sequence
			int chunkOffset, numChunks;

			// number of instances
			int instances;

			// big triangles sequence
			int bigOffset, numBigTriangles;

			// convex pieces sequence
			int convexOffset, numConvexPieces;
