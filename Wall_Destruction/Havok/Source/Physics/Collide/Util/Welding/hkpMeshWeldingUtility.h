...
class hkpMeshWeldingUtility
{
	public:
		...
		static void HK_CALL computeWeldingInfo( hkpShapeCollection* collection, const hkpBvTreeShape* bvTree, hkpWeldingUtility::WeldingType weldingType, bool weldOpenEdges = true );
		...
		struct ShapeInfo
		{
			...
			/// (hkpSimpleMeshShape, hkpExtendedMeshShape, hkpMeshShape and hkpTriSampledHeightFieldBvTreeShape
			/// implement this interface.  This method must be called after all child shapes (subparts or triangles) have 
		...
		static void HK_CALL computeWeldingInfoMultiShape( const hkTransform& meshTransform, hkpShapeCollection* mesh, hkpWeldingUtility::WeldingType weldingType, hkArray< ShapeInfo >& allShapes, bool weldOpenEdges = true );		
