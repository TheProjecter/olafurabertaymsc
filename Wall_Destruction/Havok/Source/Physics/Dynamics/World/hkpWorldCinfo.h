...
class hkpWorldCinfo : public hkReferencedObject
{
	...
	public:
		...
		hkBool m_useKdTree; //+default(false)
			...
			/// Enables use of Multiple Trees(KD-tree for active object and Aabb Tree for inactive object).
		...
		enum TreeUpdateType
		{
			...
			/// When REBUILD_ALL is set, it only uses KD-tree and rebuilds everything.
		...
		hkBool m_processToisMultithreaded;	//+default(true)
			...
			/// Note that setting this value adequately for a given simulation case may significantly improve the CPU performance.
			/// This now defaults to 8 on non-PlayStation(R)3 platforms and 4 on PlayStation(R)3 (see hkpWorldCinfo constructor).
			/// This value should be lower when we expect complicated collisions between e.g. a list shape and a complex landscape object. On contrary it should be set 
			/// to higher values when only simple objects are to be collided. Generally we need to balance the gain from offloading computations to other threads against
			/// the costs of creating and managing MT jobs.
		int m_maxEntriesPerToiCollideTask; //+default(1)

			/// The maximum number of post-Toi collision pairs processed single-threadedly.
		int m_maxNumToiCollisionPairsSinglethreaded; //+default(1)
