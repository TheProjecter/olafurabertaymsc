...
class hkpConstraintInstance : public hkReferencedObject
{
	...
	public:
		...
		enum CloningMode
		{
				/// Default behavior; create a shallow clone of the constraint unless the second body is NULL
		...
		hkpConstraintInstance(hkFinishLoadedObjectFlag f) : hkReferencedObject(f), m_name(f), m_uid(0xfffffff0) { }
			...
			///
			/// If entity B is NULL (ie: the constraint is fixed to the world) a shallow copy of the constraint
