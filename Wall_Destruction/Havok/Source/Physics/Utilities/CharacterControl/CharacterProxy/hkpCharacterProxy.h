...
public:
	...
	/// The command and maxInteractions argument are used by the hkpCharacterProxyIntegrateJob. Note: interactions are stored in the command
	/// and should be applied after the multithreaded phase. This may lead to some interpenetration.
	void integrateMultithreaded(const hkStepInfo& stepInfo, const hkVector4& gravity, hkpCharacterProxyIntegrateCommand* command , int maxInteractions );
...
protected:
	...
	/// This is a helper function for linear casting a multithreaded character proxy so that broadpahse aabbs are not updated.
	/// It performs a linear cast using the given hkpLinearCastInput and stores the results in castCollector (and startCollector if provided).
	/// If doWorldCast is true, this will call the world's linearCast() function using the shape phantom's collidable.
	/// If false, this will use the shape phantom's linear cast function much like the shape phantom's setPositionAndLinearCast() 
	/// function without setting the position or updating the broadphase.
	void linearCast( hkBool doWorldCast, const hkpLinearCastInput& input, hkpAllCdPointCollector& castCollector, hkpAllCdPointCollector* startCollector );

	/// Update the manifold of contact points without calling collision detection
	virtual void updateManifold(const hkpAllCdPointCollector& startPointCollector, const hkpAllCdPointCollector& castCollector, hkBool isMultithreaded = false );

	/// Apply impulses to the object. The command and maxInteractions arguments are used if multithreading.
	...
	void applySurfaceInteractions( const hkStepInfo& info, const hkVector4& gravity, hkpCharacterProxyIntegrateCommand* command, int maxInteractions );
	...
	void integrateImplementation(const hkStepInfo& stepInfo, const hkVector4& gravity, hkpAllCdPointCollector& castCollector, hkpAllCdPointCollector& startPointCollector, hkpCharacterProxyIntegrateCommand* command, int maxInteractions );
